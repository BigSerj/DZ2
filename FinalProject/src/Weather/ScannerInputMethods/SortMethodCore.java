...
public class SortMethodCore {
    ...
    private static Integer caseTag(ArrayList<Weather> weatherThis, int i){
        switch (constTagThis){
            case Constants.TEMP_MAX_TAG:return weatherThis.get(i).getTempMax();
            case Constants.TEMP_MIN_TAG:return weatherThis.get(i).getTempMin();
            case Constants.HUMIDITY_TAG:return weatherThis.get(i).getHumidity();
        }
        return -1;
    }
